...
import MainzCardList from './MainzCardList';

export default function MainzRestaurant({ filteredData }) {
  const [isLoading, setIsLoading] = useState(true);
  const [selectedOption, setSelectedOption] = useState('All'); // Initial state
  console.log('vibelrestaurant',filteredData);
  ...
  return (
    <>
      <div className='bg-white h-auto px-4 '>
      ...
      {isLoading ? (
            ...
            <div className="grid grid-cols-1 py-4 h-auto bg-white md:grid-cols-1 lg:grid-cols-1 px-12">
              {console.log('restaurantData in CardList passdown:',filteredData)}
              <MainzCardList data={filteredData} />
            </div>
          )}
        </div>  
    </>
